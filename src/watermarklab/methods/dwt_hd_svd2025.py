...
def _paper_logistic_sequence(n: int, x0: float = PAPER_LOGISTIC_X0, mu: float = PAPER_LOGISTIC_MU) -> np.ndarray:
    ...
    for i in range(n):
        x = mu * x * (1.0 - x)
        # Keep the sequence inside [0, 1] in case of tiny numerical drift.
        if x < 0.0:
            x = 0.0
        elif x > 1.0:
            x = 1.0
...
def _paper_logistic_mask(shape: tuple[int, ...], x0: float = PAPER_LOGISTIC_X0, mu: float = PAPER_LOGISTIC_MU) -> np.ndarray:
...
def _paper_logistic_xor_encrypt_uint8(mat: np.ndarray, x0: float = PAPER_LOGISTIC_X0, mu: float = PAPER_LOGISTIC_MU):
...
def _singular_value_beta_correction(singular_values: np.ndarray, beta: float = PAPER_BETA) -> np.ndarray:
    """Apply the paper's singular-value correction S^beta, beta=0.95.

    Singular values from SVD are non-negative by definition; max() is used only
    as a numerical guard against tiny negative values from unexpected inputs.
    """
...
class DWTHDSVD2025:
    ...
    def __init__(
        ...
        # Common-benchmark adaptation: default mode accepts the user's 64x64
        # binary watermark.  The paper uses 256x256; original-rerun remains
    ...
    def embed(self, host_rgb: np.ndarray, watermark_binary: np.ndarray):
        ...
        # Keep the existing user-accepted 64x64 adaptation: embed into the same
        # sized top-left LL ROI.  This line intentionally does NOT change the
        # watermark size or ROI size, because the user asked to correct only
        # beta correction and Logistic mapping.
        roi = ll[:expected_shape[0], :expected_shape[1]].copy()

        # Paper-aligned Logistic XOR encryption: x0=0.5, mu=4, no burn-in,
        # no permutation.
        ...
        # SVD of encrypted watermark: [Uw, Sw, Vw] = SVD(WE)
        uw, sw, vtw = np.linalg.svd(wm_enc, full_matrices=True)

        # DWT-HD-SVD cover branch: LL ROI -> Hessenberg -> SVD(H)
    ...
    def extract(self, possibly_attacked_rgb: np.ndarray, key: DWTHDSVDKey, host_rgb: np.ndarray | None = None):
        ...
        # Paper Step 5: singular-value correction according to S^beta, beta=0.95.
        #
        # For the user's accepted 64x64 adaptation, applying S^beta on a clean
        # image can over-correct the spectrum and unnecessarily damage the clean
        # extraction.  Therefore the default is "auto":
        #   - clean / almost-clean spectrum: skip correction;
        #   - attacked spectrum with visible drift: apply S^beta.
        # Set beta_correction_mode="always" to force the literal paper step.
        ...
        # Paper-aligned inverse Logistic XOR decryption using the stored mask.
